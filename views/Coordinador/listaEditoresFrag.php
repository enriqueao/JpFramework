<div id="barraOpciones">
    ...
    <select id="filtro" required name="buscar" onchange="setFiltro()">
        ...
        <option value="us.nombre">Nombre</option>
    ...
    <button id="btnBuscar" name="evaluadores" onclick="buscador('editores')"><i class="fa fa-search" aria-hidden="true"></i></button>
    <?php if(Session::getValue("tipoUsuario") != "Administrador Visual"){?>
      <button class="btnOpcion tooltipbottom" onclick="generarForm('Coordinador', 'nuevoEditor')"><i class="fa fa-user-plus" aria-hidden="true"><span class="tooltiptextbottom">Agregar Editor</span></i></button>
    <?php } ?>
</div>
...
<div id="tablaArticulos" class="tablaArticulosInfo">
        ...
        <tr>
          ...
          <?php $color = "";$idUsuario = "";?>
          <?php foreach($this->editores as $key => $valor):?>
